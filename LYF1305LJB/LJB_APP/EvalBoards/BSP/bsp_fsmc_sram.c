/*
	FileName : bsp_fsmc_sram.c
	Description:
		Use IS64WV51216BLL-10CTA3.
		如何使用外部 SRAM：
			方法一：利用指针，见本文档中的相关函数；
			方法二：
				1、生成单独一个 Variables.c 文件，将需要放置在外部 SRAM 中的所有变量定义在此文件中；
				2、在项目属性的 TARGET 页 ，在 Read/Write Memory Area 栏，定义 RAM1 内存的 start & size 为
					外置内存起始地址和长度;
				3、打开文件 Variables.c 的属性页，在 Memory Assignment 栏中，将
					Zero Initialized Data 和 Other Data 都选为 RAM1；
				4、和操作内部 RAM 数据一样操纵 Variables.c 中的变量。
