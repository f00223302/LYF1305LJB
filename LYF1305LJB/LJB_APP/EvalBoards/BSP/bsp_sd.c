/*
	FileName : bsp_sd.c
	Description : 
		1、将卡容量表示成扇区数量：因32位数只能表示4G，因此，无论标准卡还是HD卡，
			容量都指扇区数；相应地，下列函数中的 xxxAddr 指 Sector 地址：
			...
			此外，在 diskio 中调用上述函数时，Sector 不能再乘以 512 了。
			
			bsp_sd.h 中定义：
				///是否采用 Sector 地址代替字节地址：
				#define SD_USE_SECTOR_ADDRESS		1
				
		2、DMA 传送时，无需每次设置中断，只需要设置 DMA 方向、地址及数据长度；
		3、对高速卡，采用 48M 时钟；
...
///传输中断是否只初始化一次：
#define SD_USE_INTERRUPT_ONCE		1
///是否允许高速卡操作：
#define SD_ENABLE_50M_SPEED			1
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  else if (CardType == SDIO_HIGH_CAPACITY_SD_CARD)
  {
    ...
    tmp = (uint8_t)((CSD_Tab[2] & 0x0000FF00) >> 8);
		...
		///对 HD 卡，容量最大达到 32G，超出表示范围。因此，此处转义表示其 Sector 数量：
