	...
	while (try--){
		...
		err = 0;
  		...
  		for (i = StartSec; i <= EndSec; i += 8)
  		{
    		...
    		if (FLASH_EraseSector(i, ((uint8_t)0x02)) != FLASH_COMPLETE)
    		{ 
...
uint8_t F205_WriteProtection(uint32_t WRPSec, uint8_t Enable)
{
	...
	while (try--){
		...
		FLASH_Unlock();
		FLASH_ClearFlag(((uint32_t)0x00000001) | ((uint32_t)0x00000002) | ((uint32_t)0x00000010) | ((uint32_t)0x00000020) | ((uint32_t)0x00000080));
...
uint8_t F205_WriteParams(uint32_t addr, uint8_t *data, uint16_t len)
{
	...
	if (F205_WriteProtection((((uint32_t)0x00000004)), DISABLE))
		return 2;
	
	if (F205_EraseSectors(((uint16_t)0x0010), ((uint16_t)0x0010)))
		return 3;
	...
	for (i = 0; i < (len >> 2); i++){
		...
		if (status == FLASH_COMPLETE){
			...
			data32 = *(volatile uint32_t*)tmpAddr;
	...
	if (F205_WriteProtection((((uint32_t)0x00000004)), ENABLE))
		return 6;

	F205_WriteProtection((((uint32_t)0x00000004)), ENABLE);
