...
const uint32_t _USART_TIME_OUT[6] = {500, 500, 500, 500, 500, 500};
...
__inline void _SetRxData(uint8_t COM, uint8_t Data, DATA_STRUCT *Info)
{
	...
	if (__RxLen[COM] > 1){
		...
		if (__RxLen[COM] > Info->len[1] + ((uint16_t)Info->len[0] << 8) + 2)
			_InitRx(COM, 0, Info);
	}
}
...
void (*Usart_RxFrameHandler[6])(DATA_STRUCT *Info) = {
