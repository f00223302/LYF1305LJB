...
void USBH_USR_EnumerationDone(void)
{
  ...
  ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press USER1...                       ");
...
* @param  None
* @retval USBH_USR_Status : User response for USER1 button
*/
USBH_USR_Status USBH_USR_UserInput(void)
{
  ...
  ///if(STM_EVAL_PBGetState(BUTTON_USER1) == RESET) 
  {
...
int USBH_USR_MSC_Application(void)
{
  ...
  case USH_USR_FS_WRITEFILE:
    ...
    ///LCD_SetTextColor(Green);
    ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
    ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press USER1 to write file");
    ///LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
    USB_OTG_BSP_mDelay(100);
    
    /*USER1 B3 in polling*/
    ///while((HCD_IsDeviceConnected(&USB_OTG_Core)) && \
     /// (STM_EVAL_PBGetState (BUTTON_USER1) == SET))          
    {
      //Toggle_Leds();
    }
    ...
    if(f_open(&file, (const TCHAR *)"0:Host_Write_Demo.TXT",FA_CREATE_ALWAYS | FA_WRITE) == FR_OK)
    { 
    ...
    ///LCD_SetTextColor(Green);
    ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
    ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "To start Image slide show Press USER1.");
    ///LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
    break;
  ...
  if (res == FR_OK) {
    while(HCD_IsDeviceConnected(&USB_OTG_Core)) 
    {
      ...
      if(line_idx > 9)
      {
        ...
        ///LCD_SetTextColor(Green);
        ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, "                                              ");
        ///LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, "Press USER1 to continue...");
        ///LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR); 
  ...
  if (res == FR_OK) {
    ...
    for (;;) {
      ...
      else 
      {
        ...
        {
          res = f_open(&file, (const TCHAR *)fn, FA_OPEN_EXISTING | FA_READ);
          Show_Image();
          USB_OTG_BSP_mDelay(100);
          ret = 0;
          ///while((HCD_IsDeviceConnected(&USB_OTG_Core)) && \
